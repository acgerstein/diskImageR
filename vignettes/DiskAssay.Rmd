...
contributors: "Inbal Hecht" & "Richard G. Fitzjohn"
date: "`r Sys.Date()`"
...
* `IJMacro`: run the imageJ analysis macro on the folder that contains the photograph to be analyzed [required]
...
* `calcMIC`: calculate the corresponding MIC50 from RAD value using built-in or provided standard curves [optional]
...
See the end of this document for the skeleton walkthrough of package use, or the document "walkthrough.R" available <a href="http://acgerstein.weebly.com/uploads/2/3/5/6/23564534/walkthrough.r">here</a> or in the diskImageR library directory after downloaded when installed through github.
...
The first step of `diskImageR` analyzes the disk diffusion photographs in imageJ. ImageJ is a free, public domain Java image processing program available for download <a href="http://rsb.info.nih.gov/ij/download.html"> here</a>. Take note of the path to imageJ, as this might be needed for the first function. If you install it to the default location (Applications folder on a Mac, Program Files folder on a PC) then you should be able to use the default settings (see below). Depending on what is already on your computer, additional programs may also need to be downloaded. On a Mac, if you do not already have Xcode you will need to download it from the Apple Developer tools <a href="https://developer.apple.com/xcode/download/">here</a>. You may be prompted to download and install other additional programs in the R console if any are required. 

The primary output of `diskImageR` is a series of PDF files that can be customized, and a CSV file with parameter estimates that can be opened and used in any approprate program (e.g., Microsoft Excel). 
...
<b> Important! </b> `IJMacro()` must run completely, without error, for everything downstream. In our experience this is the most likely step for errors to occur. Errors should be indicated in the R console should they arise, and will hopefully give you clues as to what the problem is if they are different than those described above. 

After `IJMacro()` has run successfully there will be three new directories created in the main project directory (*image_out*, *parameter_files* and *figures*). The output of the ImageJ analysis can be found in the "imageJ_out" directory, but is probably not particularly helpful unless you want to see the intensity calculations from each line. The information about the average line from each photograph can be found in the "averageLines" file located in the "parameter_out" folder. This is the information that is used for all further analysis within `diskImageR` and may be useful for other purposes.

To access the output of `IJMacro()` in a later R session use the function `readInExistingIJ()`. At this step you can also change the project name, it does not have to be the same name that was used originally. This will also create new subdirectory folders within the main project directory. This function will bring up a pop-up box to select the main project folder and select the directory that contains the existing imageJ output files.
...
* <b>Tolerance (<i>FoG</i>)</b>
	: the `rollmean()` function from the `zoo` package is used to calculate the area under the curve from the disk edge to each RAD cutoff value. This achieved growth is then compared to the potential growth, namely, the area of a rectangle with length and height equal to RAD. The calculated paramaters are thus the fraction of full growth in this region (*FoG80*, *FoG50*, *FoG20*).
...
For this example I am loading an existing dataset that I call `manyReps.df`. This dataset contains data for seven different lines, with twelve replicates per line, and a factor I'm interested in that has two levels. I then use `aggregateData()` to average among the 12 replicates and calculate their standard error. Note that this function is generic - as long as you save your dataframe with ".df" at the end of the name you can run `aggregateData()` on any dataframe, not just one generated with `createDataframe()` or imported with `readExistingDF()`. `aggregateData()` will save a CSV file into the *parameter_files* directory as well as a new dataframe to the console (`manyReps.ag`).
...
##Calculate MIC
The function `calcMIC` is used to convert the RAD values calculated here into the typical MIC values you would acquire with a broth microdilution assay (or an Etest strip). This conversino can be based on a) existing built-in data from a nubmer of species/drug combinations (see below), b) a user-supplied slope and intercept of the linear relationship between RAD^2 and log2(MIC) for their species/drug combination of interest, c) a user supplied file containing MIC information from lines previously analyzed by diskImageR for RAD, or d) a user supplied file containing both RAD and MIC information. A column containing the MIC information is added to the dataframe and the dataframe is saved. If the user has supplied their own MIC data the function will also plot and save the calculated model parameters in a separate file.

Either a normal diskImageR dataframe (e.g., newProject.df) or aggregated dataframe (e.g., newProject.ag) can be used.

The function asks the user for input on whether to use built-in data for existing species/drug combination, or the type of data that will be supplied. Note that for user-supplied data (c or d above) the data should not already be transformed. 


##Plot parameter results
Three related plotting functions are included with `diskImageR` that use the dataframes generated `createDataframe()` and `aggregateData()`. The function `oneParamPlot()`will plot any of the single parameters (argument `param` supports "RAD20", "RAD50", "RAD80", "FoG20", "FoG50", "FoG80", "slope", the default = "RAD20") while `twoParamPlot()` will plot RAD and FoG at specified cutoff values (`RAD` supports "RAD20", "RAD50", "RAD80"; `FoG` supports "FoG20", "FoG50", "FoG80"), and `threeParamPlot()` will plot RAD, FoG and slope. 
...
`oneParamePlot()` will plot either a barplot (argument `barplot = TRUE`) or a dotplot (argument `barplot=FALSE`). In the two and three parameter plots the default is to plot FoG as a barplot and RAD and slope as a dotplot, though FoG can also be plotted as a dotplot with argument `barplot=FALSE`. 
...
###Run this function only the first time you want to run diskImageR:
...
#Use maximum likelihood to fit single and double logistic models to the data from each photograph. "clearHalo" is used to specify a picture that has a clear halo; this is used to standardize all photographs and will be most effective when photographs are taken with equal lighting without shadows. This can be a blank plate with just a disk on it (removed in the next step).
maxLik("newProject", clearHalo=1, RAD="all")

#Use the models to calculate resistance (20%, 50% and 80% reduction in growth = RAD20, RAD50, RAD80), tolerance (actual fraction of growth achvied above RAD relative to potential growth = FoG20, FoG50, FoG80), and sensitivity (slope at RAD50), which are saved in a CSV file. If you used a blank plate for clearHalo, remove with argument removeClear = TRUE.
