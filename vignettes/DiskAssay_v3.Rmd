...
New parameters are available for quantiication of observed photographs that follow either a confounding or paradoxical pattern of drug response (see images at the top of the vignette). To use this analysis, everything remains the same as descibed above until after `IJMacro()` has been run. 
...
The following curves are fit to the average line from each photgraph (determined through `IJMacro()`:
...
<b>Confounding growth </b> -- to test for confounding growth (high growth only at high drug concentrations), a <span style = "color:red;">XXX</span> equation is fit, where asym is the asymptote, od50 is the midpoint, and scal is the slopes at od50 divided by asym/4.

$$
y = \frac{asym*exp(-scal(x-od50))}{1+exp(-scal(x-od50))}+N(0, \sigma)
$$
<b>Paradoxical growth </b> -- to test for paradoxical growth (high growth at both high and low drug concentration), a <span style = "color:red;">XXX</span> equation is fit. The parameters of this equation are the same as in the logistic, as well as drop,the change between min and max values, slope, the slope of the drop, shift, the movement left/right, and height,the movement up/down.

<span style = "color:red;">XXX</span>
Nazli - here and everywhere else in this document, can you please add <i> </i> around all of the equation parameters?. e.g., 
where <i>asym</i> is the asymptote, <i>od50</i> is the midpoint, and <i>scal</i> is the slopes at <i>od50</i> divided by <i>asym</i>/4.
</span>
...
Wait, I thought this is when we needed standType = "indiv"? What you have below should get worked into the discsussion of standType above, where I also said that it couldn't be used if you wanted to calculate tolerance
...
* RAD80, RAD50, RAD20: calculated by asking what x value (distance in mm) corresponds to the point where 80\%, 50\% and 20\% reduction in growth occurred (i.e., the <u>RAD</u>ius of inhibition).

* FoG80, FoG50, FoG20: the `rollmean()` function from the `zoo` package is used to calculate the area under the curve from the disk edge to each RAD cutoff value. This achieved growth is then compared to the potential growth, i.e., the area of a rectangle with length and height equal to RAD. The calculated parameters are thus the <u>F</u>raction <u>O</u>f full <u>G</u>rowth in this region (FoG80, FoG50, FoG20). 
For confounding growth, the parameters are:

* DRAD80, DRAD50, DRAD20: calculated by asking what x value corresponds to the point where 80\%, 50\% and 20\% of maximum growth occurred (<u>D</u>isinhibition <u>RAD</u>ius).

For paradoxical growth, the parameters are:
* CMI: calculated from the <u>C</u>urve determined in `maxLik()`, the point of <u>MI</u>nimum growth from.
* OMI: calculated from the <u>O</u>bserved average data, the point of <u>MI,</u>nimum growth.
    
A different dataframe will be created individually for each type of growth that is observed in the data set, i.e., up to three dataframe. Each dataframe is written to a CSV file in the *parameter_files* directory, as well as saved to the global environment. 

<span style = "color:red;">What are the filenames and what are they saved as in the global enviornment?</span>
    
```{r}
# createDataframe("projectname", typical = TRUE, clearHalo = 1)
```
...
<span style = "color:red;"> This isn't really clear - I think you need to specify exactly which functions are considered downstream. Also, Nazli, you could try and work on updating aggregateData() in the next few days! That shouldn't be too hard :-)</span>
...
#[OPTIONAL] Calculate the mean and variance for parameter estimates across replicate photographs
