...
```{r, echo=FALSE}
library(devtools)
devtools::install_github("acgerstein/diskImageR", build_vignettes = FALSE)
library("diskImageR")
```

##<i>diskImageR</i>: Quantification of resistance and tolerance to antimicrobial drugs using disk diffusion assays

##Supplementary File S1. <i>diskImageR</i> tutorial
...
###<i>diskImageR</i> function overview
...
* `plotRaw`: used to plot the results of ImageJ analysis 
* `maxLik`: maxmimum likelihood inference to fit logistic models to the data 
* `saveMLParam`: save the output of maximum likelihood analysis to a .csv file 
* `createDataframe`: dataframe creation of all parameter from growth inhibition estimates using maxLik [RAD, ZOI, FoG]
* `inhibGrow' : dataframe creation of parameter estimates using global maximum [maxIntensity, resource limitation points]
* `addType`: add a factor column to parameter estimate dataframe 
* `aggregateData`: averages data from photographs of the same strain & type
* `calcMIC`: calculate the MIC from RAD values 
* `readExistingDF`: read in an existing dataframe using a pop-up box 
* `oneParamPlot`: plot a single resistance or tolerance parameter 
* `twoParamplot`: plot resistance (RAD) and tolerance (FoG) at a specified cutoff value 
* `threeParamplot`: plot resistance (RAD), tolerance (FoG), and sensitivity (slope) 
...
###Required software
If you do not have R (or R Studio) installed on your computer, that is step one. The use of `diskImageR` does not require prior knowledge of R and it is the goal of this tutorial to walk through everything that is necessary to analyze photographs of disk diffusion assays. 

To load the stable version of the package from CRAN (The Comprehensive R Archive Nework):
```r
install.packages("diskImageR")  #only required the first time
library("diskImageR")
```

To load the most current version of the package that is hosted on github:
```r
...
Once the package has been loaded into R through either CRAN or github, support for all built-in function can be found by typing `?functionName` in the R console.
...
```{r, echo=FALSE}
IJMacro("inhibGrowth", diskDiam = 12, projectDir= getwd(), photoDir = file.path(getwd(), "picturesInhib", ""))
```
...
###Run the ImageJ macro on 16 disk photographs 

<center>
<img src="pictures16/disk16.jpg"  style="width: 60%; height: 60%" style="float:center," alt="" />
</center> 

`diskImageR` is now able to analzye disk diffusion plates with 16 disks on them. The same plate and photograph preparatory steps should be taken. For these square plates it will also be helpful to ensure that the photographs are taken to ensure easy (square) cropping, as you will want to crop the sizes of the plate out as much as possible (as above).

For these photographs the first function of the package to run is `IJMacro16()`. From each photograph, an ImageJ macro that is included in `diskImageR` will automatically open each photograph from the specified directory, determine where the 16 disks are located on the plate, find the center of each disk, and draw 40mm radial lines out from the center of each disk every 2 degrees. 

For each line, the pixel intensity will be determined at many points along the line using the built-in `plot profile` macro from ImageJ. This data will be stored in the folder *ImageJ_out* on your computer, with one file for each photograph. Unlike in the stardard execution for single disk plates, lines are drawn every 2 degrees (instead of every 5 degrees) from the center of the disk, yielding data from 180 lines. This is to correct for additional complications from mulit-disk plates. Disks on the outside of the plates (particularly those in the corners) will have fewer lines with full profiles that are not influenced by the edges. There is also the strong potential influence of drug interactions, yielding lower intensity areas (as can be seen above for NA30 and IPM10. To correct for these complications, rather than averaging across all lines, the average pixel intensity is determined from the data at each point that corresponds to within the 0.55 and 0.75 quantile intensities. 

The order of disks will need to be specified when running `IJMacro16()` using the "drugs" flag. if you run the ImageJ macro through the user-interface with pop-up boxes you will be prompted whether your disk order matches the built-in default. Otherwise (if it doesn't) you will have to specify it manually.

The default disk order is: AMC30, CTX30, S10, CIP5, CAZ30, NA30, IPM10, AZM15, C30, FOX30, TET30, F300, ATM30, AM10, CPD10, SXT25

The majority of functions from this point are similar, with only minor flag adjustments (specified when necessary).

To run the ImageJ16 macro through a user-interface with pop-up boxes: 

```r
IJMacro16("newProject")
```

If you would prefer to avoid pop-up boxes you can directly specify the main project, photograph directory locations and disk order (if your disk order matches the default, you can leave this blank):
```r
IJMacro16("newProject", projectDir= "/path/to/projectDir", photoDir = "/path/to/projectDir/photographs/", drugs = c("drugA", "drugB", "drugC", "drugD", "drugE", "drugF", "drugG", "drugH", "drugI", "drugJ", "drugK", "drugL", "drugM", "drugN", "drugO", "drugP"))
```

```{r, echo=FALSE}
IJMacro16("newProject16", projectDir= getwd(), photoDir = file.path(getwd(), "pictures16", ""))
```
...
From these functions the plate background intensity is substracted off the intensity from all values. This can either be common across all pictures taken at the same time (`stand = "one"`; this is the default) or it can be done individually for each disk (`stand = "indiv"`). If you are using plates with different coloured base medium their photographs should be analzyed individually, as there will be a different background intensity from different plates. The background intensity is determined from the observed pixel intensity right beside the disk on a plate where there are no colonies in this area (e.g., the photograph on the left at the top). This must be specified by the user through the argument `clearHalo = X`, where X is the numbered location of the appropriate photograph. Photographs are always analyzed and organized in alphabetical order; the order can be determined by typing `names(newProject)` (no quotation marks around the project name) in the R console. In our experiments we tend to have at least one appropriate photograph with a clear halo beside the disk. A good practice, however, would be to always take a photograph of a blank plate with just the disk in the center to use for this purpose (and save it with a name like "a" so that it is always the first photograph in the list (i.e., `clearHalo = 1`). The (non)results from this photograph can be removed in the function `createDataframe()` below.
...
Note: plotting FoG has not been implemented when individual plate standards are used.

```{r,  fig.width=5, fig.height=4}
maxLik("newProject", standType="one", clearHalo=1, RAD="all", FoG=20, needML=TRUE, overwrite = TRUE, popUp = FALSE, savePDF = FALSE)
```

If you are working with 16 disk plates, you need to set the flag `needMap= TRUE`. This will tell `maxLik()` to load the disk coordinates map that was saved from `IJMacro16()` to assign the correct data to each disk.

```{r,  fig.width=7, fig.height=6}
maxLik("newProject16",  standType="indiv", maxDist = 25, RAD="all", FoG=20, needML=TRUE, overwrite = TRUE, popUp = FALSE, savePDF = FALSE, needMap = TRUE)
```
...
###Create and save a dataframe of parameter estimates using maxLik
The last required step is typically to determine the parameter estimates. The standard workflow will run the function `createDataframe()` to create and save a dataframe with the drug response parameter estimates, using the best fit parameters from the logistic equations
...
### An alternative workflow is provided to characterize growth for compounds that both inhibit growth yet are also utilized

<center>
<img src="picturesInhib/picInhib.JPG"  style="width: 40%; height: 40%" style="float:center," alt="" />
</center> 

In comparison to typical drugs, the growth profile for these compounds don't reach an asymptote, but rather are characterized by inhibition, followed by a point of maximum growth and then a growth reduction.

```{r, echo=FALSE}
plotRaw("inhibGrowth", showNum = FALSE, popUp = FALSE, savePDF = FALSE, xmax=30, plotStandardLoc = FALSE, nameVector = FALSE)
```

These compounds can still be analyzed in the traditional workflow with `maxLik()` (typically with the flag `testInhib = TRUE`, which will force the asymptote to be near the point of maximum pixel intensity). 

The fuction `inhibGrow()` can also be used to determine the grwoth maximum (distance from disk as well as pixel intensity) as well as various inhibition points beyond the growth maximum. Slopes are also calculated from the minimum intensity to the point of maximum intensity ('slope2Max') and the point of maximum intensity to the edge of the plate ('slopeFromMax').

```{r, echo=FALSE}
inhibGrow("inhibGrowth", diskDiam =12.7)
inhibGrowth_inhib.df
```
