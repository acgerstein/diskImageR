...
<!-- Note to Aleeza: for this to run properly, new pictures, need to be put into the 'pictures' file along with the vignette. Don't know how to do this automatically when merging so I think it's best for you to do it so this doesn't get overlooked. -->

<span style = "color:red;">
### Update to <i>diskImageR</i> (August 2023)
<i>diskImageR</i> is able to quantify parameters for plates with patterns of confounding (high growth at high drug concentration) and paradoxical drug response (high growth at both high and low concentration), as shown in the pictures below. For more information about the developed version, see the bottom of the vignette. </span>

<center>
<img src="new/SOR_A12.jpg"  style="width: 35%; height: 35%" style="float:left," alt="" /> <img src="new/CAY6432_Itra_R1.JPG"  style="width: 35%; height: 35%" style="float:left," alt="" />
...
<span style = "color:red;">Updates were incorporated into `maxLik()`,`saveMLParam()`, and `createDataframe()` in August 2023 to quantify different types of drug responses. All functionality has been preserved in the new versions, but the old versions are still available within the package. You can find new versions of `maxLik()`,`saveMLParam()`, and `createDataframe()`functions on Github .</span>
...
<span style="color: red;"> As of August 2023, <i>diskImageR</i> is able to standardize the pixel intensity in two different ways. The first method is to use one photo with clear background near the drug disk (described below). The second method is to standardize each photo individually based on the minimum pixel intensity measured. To use the first method, use the argument `standType = "one"` and follow the instructions below. To use the second method, use `standType = "indiv"`. The default is the first method. <span>

<span style="color: red;">If using `standType = "one"`, <span> 
...
### Update to <i>diskImageR</i>: quantifying different types of drug response

This functionality is available for use if you have observed photos that follow either a confounding or paradoxical pattern of drug response (pictures at the top of vignette). To use this updated functionality, prepare the plates and photographs following the instructions detailed above. `IJMacro()` must also be run as explained above. The next steps will be different: 

| 1. `maxLik()`: To use `maxLik()` for this purpose, use the argument `typical = FALSE`. When `typical = FALSE`, `maxLik()` considers all three types of drug responses and will choose the one with the best fit for each individual photo. 
<b>NOTE:</b> The pictures do not have to be separated manually into the three different types of drug response, `maxLik()` can be run on all photos at the same time. The following curves are fit to the data:

For plates with standard growth (high growth at low drug concentration): see double logistic equation above.

For plates with confounding growth (high growth at high concentration): parameters of this equation are as follows: asym is the asymptote, od50 is the midpoint, scal is the slopes at od50 divided by asym/4.

$$
y = \frac{asym*exp(-scal(x-od50))}{1+exp(-scal(x-od50))}+N(0, \sigma)
$$
For plates with paradoxical growth (high growth at both high and low concentration): parameters of this equation are as follows: drop is the change between min and max values, slope is the slope of the drop, shift is the movement left/right and height is the movement up/down.
...
$$
<b>NOTE: </b> `standType = "indiv"` is not supported when `typical = FALSE` (see above section on `maxLik()` for an explanation on the different options for the `standType` argument).
 
```{r}
# maxLik("projectname", typical = FALSE, clearHalo = 1)
```

| 2. `saveMLParam()`: when `maxLik()` is run with `typical = TRUE`, it will create two lists with maximum likelihood parameters, one with single logistic parameters (named ML) and one with double logistic parameters (named ML2). However, when `maxLik()` is run with `typical = FALSE`, it will only create one list, named ML2 which includes the type of curve chosen. The function `saveMLparam()` is updated to be able to handle this discrepancy and save maximum likelihood parameters in either case without error.

```{r}
# saveMLParam("projectname", typical = FALSE)
```

| 3. `createDataframe()`: if `maxLik()` is run with `typical = FALSE`, you must do the same with `createDataframe()`. `createDataframe()` will categorize the photos based on their type (standard, confounding, or paradoxical). A dataframe with the relevant parameters will be created for each category that has photos. For standard curves, the parameters are:

* RAD80, RAD50, RAD20: calculated by asking what x value (distance in mm) corresponds to the point where 80\%, 50\% and 20\% reduction in growth occurred (radius of inhibition).
...
For confounding curves, the parameters are:

* DRAD80, DRAD50, DRAD20: calculated by asking what x value corresponds to the point where 80\%, 50\% and 20\% of maximum growth occurred (radius of disinhibition).

For paradoxical curves, the parameters are:

* CMI: the point of minimum growth from the curve calculated in `maxLik()`.

* OMI: the point of minimum growth from the observed data points.
    
We are interested in adding more parameters for the paradoxical curves in future updates. 

Each of the dataframes will also contain the index of the photo for organizational purposes. Once the dataframes are created, they will be written to the global environment and .csv files. Up to three dataframes will be created in this way. 
    
```{r}
# createDataframe("projectname", typical = FALSE, clearHalo = 1)
```
