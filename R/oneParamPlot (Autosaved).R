#` Used to plot single parameter results

#' @description This function creates a pdf figure of plots showing the results of the imageJ analysis for resistance (ZOI) and tolerance (AUC), and sensitivity (slope).

#' @inheritParams plotRaw
#' @inheritParams twoParamPLot

oneParamPlot <- function(projectName, type, param  = "ZOI20", ymin = 0, ymax = 100, width = 6, height = 4, xlabels="line", xlabAngle=NA, order=NA, orderFactor = "line", overwrite=TRUE, savePDF= TRUE, popUp = TRUE, tolHist = TRUE){
...
###WORK ON THIS
	if(param %in% c("fAUC20", "fACU50", "fAUC80") & tolHist = TRUE){
