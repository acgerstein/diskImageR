		...
		if (i > length(dir())){
		 ...
		 names(newd)[3:180] <- paste0("L",2:179)
		  # aveSorted <- apply(newd, 1, function(x) mean(sort(x)[(180-numTop):180]))
			...
			sum1 <- apply(newd[2:180], 2, function(x) sum(x[1:length(x)/2.2]))
			sum2 <- apply(newd[2:180], 2, function(x) sum(x[(length(x)/2.2): length(x)]))


			close <- which(sum1 > quantile(sum1, 0.75) & sum1 < quantile(sum1, 0.85))
			far <- which(sum2 > quantile(sum2, 0.75) & sum2 < quantile(sum2, 0.95))
			aveSorted_close <- apply(newd[1:(length(newd[,1])/2.2), close], 1, function(x) mean(x))
			aveSorted_far <- apply(newd[(length(newd[,1])/2.2):length(newd[,1]), far], 1, function(x) mean(x))
